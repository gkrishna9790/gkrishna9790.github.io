...
As a first step, a random forest model was built on the sub_training_data with 53 features included. The model was used to predict the sub_testing_data . Accuracy of the model was found to be 99.23%.
...
The out of sample error is calculated as: (Misclassifed Outcomes)/(Total number of outcomes)

```{r eval=TRUE, echo=TRUE}
#6.3 Classification error
number_of_matching_values = pred_rf==sub_testing_data$classe
Predction_Error= 1 - sum(number_of_matching_values)/length(pred_rf)
Predction_Error
```
```{r eval=TRUE, echo=TRUE}
##[1] 0.003568697
...
The out of sample error was found out to be 0.3%, which is very small. So the model performs a very good job in classifying.

####**7.  Model 2: Gradient Boost model :**

To explore the scope of the improvement from the exhaustive RF model, a gradient boost algorithm was tried and tested on the sub_testing_data. However, the model failed to outperform the RF model, yielding an accuracy of 96.35%%.
...
number_of_matching_values_gbm = pred_gbm==sub_testing_data$classe
Predction_Error= 1 - sum(number_of_matching_values_gbm)/length(pred_rf)
Predction_Error
```
```{r eval=TRUE, echo=TRUE}
##[1] 0.03517716
...
The out of sample error was found out to be 3.5%, which is very high compared to the RF model. It still does a decent job in classification.
...
Since the accuracy of the combined model is same as that of the RF model, the out of sample error of the combined model would be same as that of the RF model. So, that is not estiamted here, but it would be 0.3%.

####**9.  Choosing the final model:**

Thus, the initial Random Forest model was chosen as the best model to predict. The RF model has the highest accuracy and the lowest out of sample error. The combined model was discarded since it would increase the computational complexity for meeting the same accuracy levels as the RF model. Testing the model on the sub_testing_data took care of the Cross Validation process.
...
qplot(roll_belt,yaw_belt,colour=classe,data=sub_testing_data,main="Actual classes")
```
